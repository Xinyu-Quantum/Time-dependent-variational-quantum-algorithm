...
circuit_elements[2] = QuantumCircuit(2*qubit_num,name = 'rzz0')
circuit_elements[2].rzz(Parameters[2],0,1)
circuit_elements[2].rzz(-1*Parameters[2],2,3)
...
circuit_elements[4] = QuantumCircuit(2*qubit_num,name = 'rzz1')
circuit_elements[4].rzz(Parameters[4],0,1)
circuit_elements[4].rzz(-1*Parameters[4],2,3)
...
for i in range(ele_num[2]):
    diff_elements[2][i] = QuantumCircuit(2*qubit_num+1,name = 'czz0'+str(i+1))
    diff_elements[2][i].cz(0,2*i+1)
    diff_elements[2][i].cz(0,2*i+2)
...
for i in range(ele_num[4]):
    diff_elements[4][i] = QuantumCircuit(2*qubit_num+1,name = 'czz0'+str(i+1))
    diff_elements[4][i].cz(0,2*i+1)
    diff_elements[4][i].cz(0,2*i+2) 
    ...
    for i in range(len(Parameters)):
        for j in range(i+1):    
            ...
            for k, circ in enumerate(circuit_matrix[i][j]):
                ...
                value += (2*obtain_counts(counts,'0')/shot_num-1)* coeff_matrix[i][j][k]
            # if(abs(value)<10**(-4)):
            #     value = 10**(-4)
            mat[i][j] = value
            mat[j][i] = value
...
def evaluate_vec(circuit_vec, para, coeff_vec, shot_num):
    ...
    for i in range(len(Parameters)):
        ...
        for k, circ in enumerate(circuit_vec[i]):
            ...
            value += (2*obtain_counts(counts,'0')/shot_num-1)* coeff_vec[i][k]
        vec[i] = value
    return vec
...
def measurement(para, shot_num):
    ...
    rho = np.sqrt(rho)/sum(np.sqrt(rho))
    mz = 0
    for i in range(2**qubit_num):
...
def diff_para(circuit_matrix, circuit_vec, coeff_matrix, coeff_vec, para, shot_num, reg):
    mat = evaluate_matrix(circuit_matrix, para, coeff_matrix, shot_num)
    mat = mat + reg*np.eye(5)
...
def TDVA_evolution(para, dt, steps, shot_num, circuit_matrix, circuit_vec, coeff_matrix, coeff_vec):
    ...
    for i in range(steps):
        mz[i] = measurement(para,shot_num)
        t[i] = i*dt
        paras[i] = para
        dpara, mat, vec = diff_para(circuit_matrix, circuit_vec, coeff_matrix, coeff_vec,para, shot_num, 0.0)
        para += dpara*dt
        if(para[0]>pi):
            para[0] = pi
        if(para[0]<0):
...
mz, t, paras, mats, vecs = TDVA_evolution(para, 0.01, 500, 20000, circuit_matrix, circuit_vec, coeff_matrix, coeff_vec)
