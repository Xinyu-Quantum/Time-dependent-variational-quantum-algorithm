...
function [U, U_diff_b] = generate_Trotter_ansatz_TXYZ(spin_num, layer, bvalue)
    Rxs = complex(zeros([spin_num*layer,2,2],'single'));
    Rzzs = complex(zeros([(spin_num-1)*layer,2^spin_num,2^spin_num],'single'));
...
function [U, U_diff_b] = generate_Trotter_ansatz_XYZ(spin_num, layer, bvalue)
    ...
    for i = 1:layer
        for j = 1:spin_num-1
            Rzzs((spin_num-1)*(i-1)+j,:,:) = rzz_multi(spin_num,j,bvalue((3*spin_num-3)*(i-1)+j));
            Ryys((spin_num-1)*(i-1)+j,:,:) = ryy_multi(spin_num,j,bvalue((3*spin_num-3)*(i-1)+j+spin_num-1));
            Rxxs((spin_num-1)*(i-1)+j,:,:) = rxx_multi(spin_num,j,bvalue((3*spin_num-3)*(i-1)+j+2*spin_num-2));
        end
    end
    Rzz1 = complex(zeros([layer,2^spin_num,2^spin_num],'single'));
    Rzz2 = complex(zeros([layer,2^spin_num,2^spin_num],'single'));
    ...
    if spin_num>2
        ...
        ULs = complex(zeros([6*layer,2^spin_num,2^spin_num],'single'));
        ULs(1,:,:) = reshape(Rzz1(1,:,:),2^spin_num,2^spin_num);
        ULs(2,:,:) = reshape(Rzz2(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(1,:,:),2^spin_num,2^spin_num);
        ULs(3,:,:) = reshape(Ryy1(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(2,:,:),2^spin_num,2^spin_num);
        ULs(4,:,:) = reshape(Ryy2(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(3,:,:),2^spin_num,2^spin_num);
        ULs(5,:,:) = reshape(Rxx1(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(4,:,:),2^spin_num,2^spin_num);
        ULs(6,:,:) = reshape(Rxx2(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(5,:,:),2^spin_num,2^spin_num);       
        for i = 2:layer
            ULs(6*i-5,:,:) = reshape(Rzz1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-6,:,:),2^spin_num,2^spin_num);
            ULs(6*i-4,:,:) = reshape(Rzz2(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-5,:,:),2^spin_num,2^spin_num);
            ULs(6*i-3,:,:) = reshape(Ryy1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-4,:,:),2^spin_num,2^spin_num);
            ULs(6*i-2,:,:) = reshape(Ryy2(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-3,:,:),2^spin_num,2^spin_num);
            ULs(6*i-1,:,:) = reshape(Rxx1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-2,:,:),2^spin_num,2^spin_num);
            ULs(6*i,:,:) = reshape(Rxx2(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(6*i-1,:,:),2^spin_num,2^spin_num);
        end

        U = reshape(ULs(6*layer,:,:),2^spin_num,2^spin_num);

        URs = complex(zeros([6*layer,2^spin_num,2^spin_num],'single'));
        URs(6*layer,:,:) = reshape(Rxx2(layer,:,:),2^spin_num,2^spin_num);
        URs(6*layer-1,:,:) = reshape(URs(6*layer,:,:),2^spin_num,2^spin_num)*reshape(Rxx1(layer,:,:),2^spin_num,2^spin_num); 
        URs(6*layer-2,:,:) = reshape(URs(6*layer-1,:,:),2^spin_num,2^spin_num)*reshape(Ryy2(layer,:,:),2^spin_num,2^spin_num); 
        URs(6*layer-3,:,:) = reshape(URs(6*layer-2,:,:),2^spin_num,2^spin_num)*reshape(Ryy1(layer,:,:),2^spin_num,2^spin_num); 
        URs(6*layer-4,:,:) = reshape(URs(6*layer-3,:,:),2^spin_num,2^spin_num)*reshape(Rzz2(layer,:,:),2^spin_num,2^spin_num); 
        URs(6*layer-5,:,:) = reshape(URs(6*layer-4,:,:),2^spin_num,2^spin_num)*reshape(Rzz1(layer,:,:),2^spin_num,2^spin_num);                 
        for i = linspace(layer-1,1,layer-1)
            URs(6*i,:,:) = reshape(URs(6*i+1,:,:),2^spin_num,2^spin_num)*reshape(Rxx2(i,:,:),2^spin_num,2^spin_num);
            URs(6*i-1,:,:) = reshape(URs(6*i,:,:),2^spin_num,2^spin_num)*reshape(Rxx1(i,:,:),2^spin_num,2^spin_num);
            URs(6*i-2,:,:) = reshape(URs(6*i-1,:,:),2^spin_num,2^spin_num)*reshape(Ryy2(i,:,:),2^spin_num,2^spin_num);
            URs(6*i-3,:,:) = reshape(URs(6*i-2,:,:),2^spin_num,2^spin_num)*reshape(Ryy1(i,:,:),2^spin_num,2^spin_num);
            URs(6*i-4,:,:) = reshape(URs(6*i-3,:,:),2^spin_num,2^spin_num)*reshape(Rzz2(i,:,:),2^spin_num,2^spin_num);
            URs(6*i-5,:,:) = reshape(URs(6*i-4,:,:),2^spin_num,2^spin_num)*reshape(Rzz1(i,:,:),2^spin_num,2^spin_num);
        end
    
        U_diff_b = complex(zeros([(3*spin_num-3)*layer,2^spin_num,2^spin_num],'single'));
        for i = 1:layer
            for j =1:spin_num-1
                if mod(j,1) == 1
                    if i == 1
                        U_diff_b((3*spin_num-3)*(i-1)+j,:,:) = -0.5*1j*reshape(URs(6*i-5,:,:),2^spin_num,2^spin_num)*sigmaz_sigmaz_multi(spin_num,j);
                    else
                        U_diff_b((3*spin_num-3)*(i-1)+j,:,:) = -0.5*1j*reshape(URs(6*i-5,:,:),2^spin_num,2^spin_num)*sigmaz_sigmaz_multi(spin_num,j)*reshape(ULs(6*i-6,:,:),2^spin_num,2^spin_num);
                    end
                        U_diff_b((3*spin_num-3)*(i-1)+spin_num-1+j,:,:) = -0.5*1j*reshape(URs(6*i-3,:,:),2^spin_num,2^spin_num)*sigmay_sigmay_multi(spin_num,j)*reshape(ULs(6*i-4,:,:),2^spin_num,2^spin_num);
                        U_diff_b((3*spin_num-3)*(i-1)+2*spin_num-2+j,:,:) = -0.5*1j*reshape(URs(6*i-1,:,:),2^spin_num,2^spin_num)*sigmax_sigmax_multi(spin_num,j)*reshape(ULs(6*i-2,:,:),2^spin_num,2^spin_num);
                else
                        U_diff_b((3*spin_num-3)*(i-1)+j,:,:) = -0.5*1j*reshape(URs(6*i-4,:,:),2^spin_num,2^spin_num)*sigmaz_sigmaz_multi(spin_num,j)*reshape(ULs(6*i-5,:,:),2^spin_num,2^spin_num);
                        U_diff_b((3*spin_num-3)*(i-1)+spin_num-1+j,:,:) = -0.5*1j*reshape(URs(6*i-2,:,:),2^spin_num,2^spin_num)*sigmay_sigmay_multi(spin_num,j)*reshape(ULs(6*i-3,:,:),2^spin_num,2^spin_num);
                        U_diff_b((3*spin_num-3)*(i-1)+2*spin_num-2+j,:,:) = -0.5*1j*reshape(URs(6*i,:,:),2^spin_num,2^spin_num)*sigmax_sigmax_multi(spin_num,j)*reshape(ULs(6*i-1,:,:),2^spin_num,2^spin_num);
                end
            end
        end
    end
    if spin_num == 2
        ...
        ULs = complex(zeros([3*layer,2^spin_num,2^spin_num],'single'));
        ULs(1,:,:) = reshape(Rzz1(1,:,:),2^spin_num,2^spin_num);
        ULs(2,:,:) = reshape(Ryy1(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(1,:,:),2^spin_num,2^spin_num);
        ULs(3,:,:) = reshape(Rxx1(1,:,:),2^spin_num,2^spin_num)*reshape(ULs(2,:,:),2^spin_num,2^spin_num);
        for i = 2:layer
            ULs(3*i-2,:,:) = reshape(Rzz1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(3*i-3,:,:),2^spin_num,2^spin_num);
            ULs(3*i-1,:,:) = reshape(Ryy1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(3*i-2,:,:),2^spin_num,2^spin_num);
            ULs(3*i,:,:) = reshape(Rxx1(i,:,:),2^spin_num,2^spin_num)*reshape(ULs(3*i-1,:,:),2^spin_num,2^spin_num);
        end

        U = reshape(ULs(3*layer,:,:),2^spin_num,2^spin_num);

        URs = complex(zeros([3*layer,2^spin_num,2^spin_num],'single'));
        URs(3*layer,:,:) = reshape(Rxx1(layer,:,:),2^spin_num,2^spin_num);
        URs(3*layer-1,:,:) = reshape(URs(3*layer,:,:),2^spin_num,2^spin_num)*reshape(Ryy1(layer,:,:),2^spin_num,2^spin_num); 
        URs(3*layer-2,:,:) = reshape(URs(3*layer-1,:,:),2^spin_num,2^spin_num)*reshape(Rzz1(layer,:,:),2^spin_num,2^spin_num);    
        for i = linspace(layer-1,1,layer-1)
            URs(3*i,:,:) = reshape(URs(3*i+1,:,:),2^spin_num,2^spin_num)*reshape(Rxx1(i,:,:),2^spin_num,2^spin_num);
            URs(3*i-1,:,:) = reshape(URs(3*i,:,:),2^spin_num,2^spin_num)*reshape(Ryy1(i,:,:),2^spin_num,2^spin_num);
            URs(3*i-2,:,:) = reshape(URs(3*i-1,:,:),2^spin_num,2^spin_num)*reshape(Rzz1(i,:,:),2^spin_num,2^spin_num);
        end
...
function [tlist, mz, mx, paras] = VQD(L, R, spin_num, para_initial, dt, steps, layer)
    ...
    for i = 1:steps
        ...
        [U, U_diff_b] = generate_Trotter_ansatz_TXYZ(spin_num, layer, paras(R+1:R+num_b,i)');
